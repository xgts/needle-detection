...
from  data.wireframe_dset import Line_Dataset, LineDataset_collate_fn
...
def get_dataset(cfg, is_train = True):
    ...
    return  __mapping_dataset[cfg.datasets.name](cfg,is_train = is_train)
...
def get_train_dataloader(cfg):
    ...
    dloader = DataLoader(ds, batch_size = cfg.train.batch_size,shuffle = True,num_workers = cfg.sys.num_workers,
                         drop_last=True,collate_fn= get_collate_fn(cfg)
    )
    return dloader
