...
from data.utils import deccode_lines
from metric import msTPFP, AP
...
def infer_one(img_fn, model, input_size=512, score_thresh=0.01, min_len=0, topk=200):
    ...
    with torch.no_grad():
        batch_outputs,seg_out = model(img)
...
def calculate_sAP(gt_infos, pred_infos, sap_thresh):
    ...
    for gt, pred in zip(gt_infos, pred_infos):
        ...
        tp, fp = msTPFP(pred_lines, gt_lines, sap_thresh)
...
def main(args):
    ...
    for c in tqdm.tqdm(contens):
        ...
        fn = c['filename'][:-4] + '.png'
        full_fn = img_dir + '/' + fn
        pred_infos.append(infer_one(full_fn, model,args.input_size,args.score_thresh,args.min_len, args.top_k ))



    ap = calculate_sAP(gt_infos, pred_infos, 10)

    print("====> sAP{}: {}".format(10, ap))
